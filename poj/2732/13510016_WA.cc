...
int main() {
	...
	for (int t = 0; t < tcase; t++) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (parentNode == NULL) {
				parentNode = makeNode(parent, d, NULL);
			}
			...
			for (int j = 0; j < m; j++) {
				...
				if (childNode == NULL) {
					childNode = makeNode(child, d, parentNode);
				}
		...
		for (int i = 0; i < graph.size(); i++) {
			if (*(graph[i]->answer) > 0) {
				...
				if (i + 1 != graph.size() && *(graph[i]->answer) == *(graph[i+1]->answer)) cnt--;
				if (cnt == 3) break;
			}
		}
		cout << endl;
